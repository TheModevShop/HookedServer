...
hooks.getById = function(id, q, position, userId) {
  q = q || {};
  const queryObject = {};
  queryObject.coordinates = [
    Number(position.lng),
  ...
  return bookshelf.knex('hooks')
  .join('merchants', 'merchants.id', '=', 'hooks.merchant_id')
  .leftJoin('redemptions', function() {
    this.on('redemptions.hook_id', '=', Number(id)).andOn('redemptions.user_id', '=', Number(userId))
  })
  .select('redemptions.created_at as redemption', 'hooks.*', 'merchants.polygon', 'merchants.name', st.area('polygon').as('area'), st.x(st.centroid('polygon')).as('x'),  st.y(st.centroid('polygon')).as('y'), st.asText('polygon'), st.distance('polygon', point))
